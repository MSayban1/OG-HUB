    ...
    <header className="sticky top-0 bg-slate-950/80 backdrop-blur-md border-b border-slate-800 z-50 px-4 h-16 flex items-center justify-between">
      <div className="flex items-center space-x-2">
        ...
        <span 
          ...
          className="text-xl font-black tracking-tight text-indigo-500 cursor-pointer hover:opacity-80 transition-all active:scale-95 select-none"
        >
          {APP_NAME}
        </span>
      </div>
      ...
      <div className="flex items-center space-x-2">
        {user && (
          <button 
            onClick={onChatClick}
            className="w-9 h-9 rounded-xl flex items-center justify-center transition-all border bg-slate-900 text-slate-400 border-slate-800 hover:text-indigo-400 hover:border-indigo-500/30 relative"
            title="Global Chat"
          >
            <i className="fa-solid fa-comments text-sm"></i>
            <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-indigo-500 rounded-full animate-pulse shadow-[0_0_5px_rgba(99,102,241,0.8)]"></span>
          </button>
        )}
          ...
          <Link 
            ...
            className={`
              ...
              ${isAdminPath 
                ...
                : 'bg-slate-900 text-indigo-400 border border-indigo-500/30 hover:bg-indigo-600 hover:text-white'
              }
            `}
          >
            {!isAdminPath && (
              <span className="absolute inset-0 rounded-xl bg-indigo-500/20 animate-ping opacity-75 pointer-events-none"></span>
            )}
        ...
        {!user ? (
          ...
          <Link to="/dashboard" className="group flex items-center">
             <div className="w-9 h-9 rounded-xl bg-slate-900 flex items-center justify-center border border-slate-800 group-hover:border-indigo-500/50 transition-colors shadow-inner overflow-hidden">
               {profilePic ? (
                 <img src={profilePic} alt="Profile" className="w-full h-full object-cover" />
               ) : (
                 <i className="fa-solid fa-user text-sm text-slate-500 group-hover:text-indigo-400 transition-colors"></i>
               )}
             </div>
          </Link>
        )}
