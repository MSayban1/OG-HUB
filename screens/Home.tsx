...
const Home: React.FC = () => {
      ...
      <section className="mb-10">
        <div className="flex items-center justify-between mb-4 px-1">
          <h3 className="text-lg font-black text-slate-50 flex items-center">
            <i className={`fa-solid ${icon} text-indigo-400 mr-2 text-sm`}></i>
            {title}
          </h3>
        </div>
        ...
        {gamesList.length > 0 ? (
              ...
              >
                <div className="aspect-[1/1] overflow-hidden relative">
                  ...
                  <div className="absolute top-4 left-4 flex flex-col gap-1.5 z-10">
                    {game.isFeatured && (
                      <span className="bg-indigo-600 text-white text-[7px] font-black uppercase px-2 py-1 rounded-md shadow-lg border border-indigo-400/20 flex items-center featured-pulse">
          ...
          <div className="bg-slate-900/50 border border-dashed border-slate-800 rounded-[2.5rem] p-16 text-center flex flex-col items-center">
            ...
            <h4 className="text-slate-50 font-bold mb-2">No Missions Found</h4>
  ...
  return (
    <div className="space-y-8 animate-fadeIn pb-10">
      <section className="relative h-48 rounded-[2rem] overflow-hidden shadow-2xl shadow-indigo-500/10 bg-slate-900 border border-slate-800">
        {banners.length > 0 ? (
          <div className="w-full h-full relative transition-all duration-700 ease-in-out">
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex space-x-1.5">
              {banners.map((_, idx) => (
                <div key={idx} className={`h-1.5 rounded-full transition-all duration-300 ${idx === currentBannerIndex ? 'w-6 bg-indigo-500' : 'w-1.5 bg-slate-600'}`}></div>
              ))}
            </div>
          </div>
        ) : (
          <div className="w-full h-full bg-slate-900 flex flex-col justify-end p-6">
            <h2 className="text-2xl font-black text-slate-50 leading-tight">Level Up Your Fun</h2>
      ...
      <section className="px-1">
        <div className="relative group">
          ...
          <input
            ...
            className="w-full bg-slate-900 border border-slate-800 text-slate-50 text-sm rounded-2xl pl-11 pr-4 py-4 outline-none transition-all focus:ring-2 focus:ring-indigo-500/50"
          />
        </div>
      </section>
