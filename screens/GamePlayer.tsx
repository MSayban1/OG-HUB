      ...
      <div className="fixed inset-0 bg-slate-950 z-[60] flex flex-col overflow-hidden page-transition">
        <div className="shrink-0 p-6 z-30 flex items-center justify-between">
          <button
            ...
            className="w-10 h-10 rounded-full bg-slate-900/90 backdrop-blur-xl border border-white/10 flex items-center justify-center text-contrast active:scale-90 transition-all shadow-2xl pointer-events-auto"
          >
          ...
          <button
            ...
            className={`w-10 h-10 rounded-full backdrop-blur-xl border flex items-center justify-center transition-all active:scale-90 shadow-2xl ${isFavorite ? 'bg-rose-500/20 border-rose-500/50 text-rose-500' : 'bg-slate-900/90 border-white/10 text-contrast'}`}
          >
        ...
        <div className="flex-1 overflow-y-auto px-6 flex flex-col items-center scrollbar-hide pb-10">
          <div className="relative w-full max-w-[240px] aspect-square mb-8 mt-2 shrink-0">
            <div className="absolute -inset-6 bg-theme-500 rounded-full blur-[80px] opacity-10"></div>
            <img
              src={game.iconUrl}
          ...
          <div className="text-center space-y-3 mb-8 shrink-0">
            <h2 className="text-3xl font-black text-contrast tracking-tighter leading-tight px-4">{game.name}</h2>
            <div className="flex items-center justify-center flex-wrap gap-2">
              <span className="text-[10px] font-black uppercase tracking-widest bg-theme-500 text-white px-3 py-1 rounded-full">{game.category}</span>
          ...
          <div className="w-full max-w-md bg-slate-900/50 border border-slate-800 rounded-[2.5rem] p-8 space-y-4 mb-6">
            <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-500 flex items-center justify-center">
              <i className="fa-solid fa-circle-info mr-2 text-theme-500"></i>
              Mission Briefing
            </h4>
        ...
        <div className="shrink-0 p-6 bg-gradient-to-t from-slate-950 via-slate-950 to-transparent pt-12 border-t border-slate-900/50">
          <button
            ...
            className="w-full max-w-md mx-auto bg-theme-600 hover:bg-theme-700 text-white font-black py-5 rounded-[1.8rem] shadow-[0_15px_40px_-10px_rgba(var(--theme-rgb-500),0.5)] active:scale-95 transition-all text-lg flex items-center justify-center space-x-4 border-b-4 border-theme-800"
          >
    ...
    return (
      <div className="fixed inset-0 bg-slate-950 z-[70] flex flex-col items-center justify-center space-y-12 px-8 overflow-hidden">
        <div className="absolute inset-0 overflow-hidden">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[150%] aspect-square bg-theme-500/10 rounded-full blur-[120px] animate-pulse"></div>
        </div>

        <div className="relative">
          <div className="w-32 h-32 border-4 border-theme-500/5 rounded-full"></div>
          <div className="absolute inset-0 w-32 h-32 border-t-4 border-theme-500 rounded-full animate-spin"></div>
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="relative group">
              <div className="absolute -inset-4 bg-theme-500/20 rounded-2xl blur-xl group-hover:opacity-100 opacity-70 transition-opacity"></div>
        ...
        <div className="text-center space-y-6 relative z-10 w-full max-w-xs">
          <div className="space-y-1">
            <h3 className="text-2xl font-black text-contrast tracking-tighter">Linking Realities</h3>
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-theme-400/80 animate-pulse">Syncing Game Data...</p>
          </div>
          <div className="w-full h-1.5 bg-slate-900 border border-slate-800 rounded-full overflow-hidden">
            <div className="h-full bg-theme-500 rounded-full w-0 animate-[progress_2.4s_ease-in-out_forwards] shadow-[0_0_15px_rgba(var(--theme-rgb-500),0.5)]"></div>
          </div>
    ...
    <div className="fixed inset-0 bg-slate-950 z-[80] flex flex-col h-full w-full overflow-hidden">
      <div className="h-14 bg-slate-900/90 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-4 shrink-0 shadow-lg">
        ...
        <div className="flex items-center space-x-3 flex-1 px-4 min-w-0 justify-center">
          ...
          <h2 className="font-bold text-sm truncate text-contrast max-w-[150px]">{game.name}</h2>
