  ...
  return (
    <div className="max-w-4xl mx-auto animate-fadeIn pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 px-4 sm:px-0">
        <div>
          <h2 className="text-3xl font-black text-slate-50 tracking-tighter">Command Center</h2>
          <p className="text-indigo-400 font-bold text-[10px] uppercase tracking-[0.2em]">Verified Hub Authority</p>
        </div>
        <div className="flex bg-slate-900 p-1 rounded-2xl border border-slate-800 overflow-x-auto scrollbar-hide">
           <button onClick={() => setTab('games')} className={`px-4 py-2 rounded-xl text-[10px] uppercase tracking-widest font-black transition-all whitespace-nowrap ${tab === 'games' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500'}`}>Missions</button>
           <button onClick={() => setTab('servers')} className={`px-4 py-2 rounded-xl text-[10px] uppercase tracking-widest font-black transition-all whitespace-nowrap ${tab === 'servers' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500'}`}>Servers</button>
           <button onClick={() => setTab('banners')} className={`px-4 py-2 rounded-xl text-[10px] uppercase tracking-widest font-black transition-all whitespace-nowrap ${tab === 'banners' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500'}`}>Banners</button>
           <button onClick={() => setTab('tickets')} className={`px-4 py-2 rounded-xl text-[10px] uppercase tracking-widest font-black transition-all whitespace-nowrap relative ${tab === 'tickets' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500'}`}>
              Intelligence
           ...
           <button onClick={() => setTab('users')} className={`px-4 py-2 rounded-xl text-[10px] uppercase tracking-widest font-black transition-all whitespace-nowrap ${tab === 'users' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500'}`}>Gamers</button>
           <button onClick={() => setTab('add')} className={`px-4 py-2 rounded-xl text-[10px] uppercase tracking-widest font-black transition-all whitespace-nowrap ${tab === 'add' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500'}`}>+ Deploy</button>
        </div>
      </div>
            ...
            {games.length > 0 ? games.map(game => (
              <div key={game.id} className="bg-slate-900 border border-slate-800 rounded-3xl p-5 flex items-center space-x-5 shadow-xl group">
                <div className="relative">
                  ...
                  {game.isFeatured && (
                    <div className="absolute -top-2 -right-2 w-6 h-6 bg-indigo-600 rounded-full flex items-center justify-center text-white text-[10px] border-2 border-slate-900 shadow-lg">
                      <i className="fa-solid fa-star"></i>
                    </div>
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <h4 className="font-bold text-slate-50 text-sm truncate">{game.name}</h4>
                ...
                <div className="flex items-center space-x-2">
                   <button 
                     ...
                     className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all border ${game.isFeatured ? 'bg-indigo-500/20 border-indigo-500 text-indigo-400' : 'bg-slate-800 border-slate-700 text-slate-600'}`}
                   ...
                   <button 
                     ...
                     className="w-10 h-10 rounded-xl bg-slate-800 text-slate-400 hover:bg-indigo-600 hover:text-white transition-all shadow-inner"
                     title="Edit Mission"
                   >
        ...
        {tab === 'servers' && (
          <div className="space-y-6">
            <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800 flex flex-col items-center text-center space-y-4">
              ...
              <h3 className="text-slate-50 font-black text-xl">Core Grid Sync</h3>
              ...
              <button onClick={handleInitializeDefaultServers} className="bg-indigo-600 text-white px-8 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest active:scale-95 transition-all shadow-lg shadow-indigo-600/20 border-b-4 border-indigo-800">Initialize Core (1-10)</button>
            </div>

            <form onSubmit={handleAddServer} className="bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800 space-y-4">
              <h4 className="font-black text-xs uppercase tracking-widest text-slate-500 flex items-center"><i className="fa-solid fa-plus-circle mr-2 text-indigo-400"></i>Custom Node Deployment</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <input type="text" required placeholder="Server Name" className="bg-slate-950 border border-slate-800 rounded-2xl px-5 py-4 outline-none text-sm focus:ring-2 focus:ring-indigo-500/50 text-white placeholder:text-slate-700" value={serverFormData.name} onChange={(e) => setServerFormData({...serverFormData, name: e.target.value})} />
                <input type="text" placeholder="Mission Briefing..." className="bg-slate-950 border border-slate-800 rounded-2xl px-5 py-4 outline-none text-sm focus:ring-2 focus:ring-indigo-500/50 text-white placeholder:text-slate-700" value={serverFormData.description} onChange={(e) => setServerFormData({...serverFormData, description: e.target.value})} />
              </div>
            ...
            <div className="grid gap-4">
              {chatRooms.length > 0 ? chatRooms.sort((a,b) => a.name.localeCompare(b.name, undefined, {numeric: true})).map(room => (
                <div key={room.id} className="bg-slate-900 border border-slate-800 rounded-3xl p-5 flex items-center space-x-5 shadow-xl">
                  <div className={`w-12 h-12 bg-slate-950 rounded-2xl flex items-center justify-center border border-slate-800 ${room.enabled ? 'text-indigo-400' : 'text-slate-700'}`}>
                    <i className="fa-solid fa-tower-broadcast"></i>
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className={`font-black text-sm uppercase tracking-tight truncate ${room.enabled ? 'text-slate-50' : 'text-slate-500'}`}>{room.name}</h4>
                    ...
                    return (
                      <tr key={u.uid} className="hover:bg-slate-800/30 transition-colors">
                        <td className="px-8 py-5">
                          <p className="font-black text-sm text-slate-50 truncate max-w-[150px]">{u.username || 'Anonymous'}</p>
                          <p className="text-[10px] text-slate-500 font-bold truncate max-w-[200px]">{email}</p>
                        </td>
                        <td className="px-8 py-5">
                          {isSystemAdmin ? (
                            <span className="px-3 py-1.5 rounded-lg font-black text-[9px] uppercase tracking-widest bg-indigo-500 text-white shadow-lg shadow-indigo-500/20">Admin</span>
                          ) : (
            ...
            {tickets.length > 0 ? tickets.map((ticket) => (
              <div key={ticket.firebaseId} className={`bg-slate-900 border border-slate-800 rounded-[2.5rem] p-6 shadow-2xl relative overflow-hidden group ${ticket.status === 'resolved' ? 'opacity-60 grayscale-[0.3]' : ''}`}>
                <div className="absolute top-0 right-0 p-6 flex flex-col items-end space-y-1">
                  <span className={`text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-lg border ${ticket.status === 'resolved' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20'}`}>ID: {ticket.id}</span>
                ...
                <div className="flex items-start space-x-5">
                    ...
                    <div className="flex-1 space-y-4 pt-1 min-w-0">
                      <div>
                        <h4 className="font-black text-slate-50 text-base truncate">{ticket.username}</h4>
                      ...
                      <div className="bg-slate-950/50 rounded-2xl border border-slate-800 p-4">
                        <p className="text-[9px] font-black uppercase text-indigo-400 mb-1">{ticket.problemType}</p>
        ...
        {tab === 'banners' && (
          <div className="space-y-6">
            <form onSubmit={handleAddBanner} className="bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800 space-y-4">
              ...
              <div className="flex gap-2">
                <input type="url" required placeholder="Image URL..." className="flex-1 bg-slate-950 border border-slate-800 rounded-2xl px-5 py-4 outline-none text-sm focus:ring-2 focus:ring-indigo-500/50 text-white placeholder:text-slate-700" value={bannerUrl} onChange={(e) => setBannerUrl(e.target.value)} />
                <button type="submit" className="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest active:scale-95 transition-all">Add</button>
              </div>
            </form>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {banners.map(b => (
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
             ...
             <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-6 sm:p-10 shadow-2xl relative w-full max-w-2xl animate-page-transition overflow-y-auto max-h-[90vh] scrollbar-hide">
              <div className="flex items-center justify-between mb-8">
                <div>
                  <h3 className="text-2xl font-black text-slate-50 tracking-tighter">{editingGame ? 'Update Mission' : 'Deploy New Mission'}</h3>
                  <p className="text-[10px] font-black uppercase text-indigo-400 tracking-widest">{editingGame ? `Edit ID: ${editingGame.id}` : 'Catalog Expansion'}</p>
                </div>
              ...
              <form onSubmit={editingGame ? handleUpdateGame : handleAddGame} className="space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-5 bg-slate-950/50 rounded-2xl border border-slate-800">
                  ...
                  <label className="flex flex-col items-center justify-center p-3 rounded-xl border border-slate-800 bg-slate-900 cursor-pointer transition-all hover:bg-slate-800">
                    ...
                    <i className={`fa-solid fa-star mb-1 ${formData.isFeatured ? 'text-indigo-500' : 'text-slate-600'}`}></i>
                ...
                <button type="submit" className="w-full bg-indigo-600 text-white font-black py-5 rounded-2xl active:scale-95 transition-all text-xs uppercase tracking-[0.2em] border-b-4 border-indigo-800 shadow-xl shadow-indigo-600/20">
